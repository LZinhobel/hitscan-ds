...
\begin{figure}[h]
	...
	\fbox{\parbox{0.92\linewidth}{
		\textbf{Image slot suggestion: physical test setup photo.}\\
		Capture a wide-angle image showing:
		\begin{itemize}
			\item full dartboard,
			\item ring light placement,
			\item camera position with measured $\approx 60\,\mathrm{cm}$ distance,
			\item approximate viewing angle annotation ($\approx 50^{\circ}$).
		\end{itemize}
	}}
	\caption{Placeholder for physical evaluation setup documentation photo.}
...
\begin{figure}[h]
	\centering
	\fbox{\parbox{0.92\linewidth}{
		\textbf{Image slot suggestion: lighting comparison pair.}\\
		Provide two board images with identical camera placement:
		\begin{itemize}
			\item left: without ring light (visible shaft shadows),
			\item right: with ring light (uniform illumination).
		\end{itemize}
	}}
	\caption{Placeholder for before/after lighting-condition comparison.}
