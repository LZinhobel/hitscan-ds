...
Cycle Input: one unstable system behavior
  Define the expected behavior and a corresponding measurable metric.
  Instrument and debug the corresponding stage (for instance, saved masks, contours, and tip output).
  Modify one parameter group at a time.
  Re-run specific controlled throw tests (center, edge, out-of-bounds).
  Log the behavior and decide to commit or revert the modification.
  Integrate only when no regression is seen on previously stable behavior.
\end{lstlisting}
