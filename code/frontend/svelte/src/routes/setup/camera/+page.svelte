...
function handleKey(e: KeyboardEvent) {
  ...
  if (mode === "rings") {
    switch (e.key) {
      ...
      case "+": case "=": r.radius += SCALE_STEP; break;
      ...
      case "n":
        ...
        if (currentRing >= NUM_RINGS) {
          console.log("Calibration done:", rings, "Line config:", { lineRotation, lineOffsetX, lineOffsetY, lineScale, lineStretchX, lineStretchY });
    ...
    switch (e.key) {
      ...
      case "r": lineRotation -= ROTATE_STEP; break;
      case "t": lineRotation += ROTATE_STEP; break;
      case "+": case "=": lineScale += STRETCH_STEP; break;
      case "-": lineScale = Math.max(0.1, lineScale - STRETCH_STEP); break;
      case "n":
        console.log("Calibration finished:", rings, { lineRotation, lineOffsetX, lineOffsetY, lineScale, lineStretchX, lineStretchY });
        break;
    }
  }
...
<div class="camera-buttons">
  {#each cameras as cam, i (cam.deviceId)}
    <button type="button" class:selected={i===selectedIndex} on:click={()=>selectCamera(i)}>{cam.label}</button>
  {/each}
</div>
...
<div class="camera-container" style="position:relative;">
  {#if selectedIndex!==null}
    ...
    <canvas bind:this={canvasElement} style="position:absolute;top:0;left:0;width:100%;height:100%;"></canvas>
  {/if}
</div>
...
.camera-buttons button{
  padding:0.5rem 1rem; cursor:pointer; border-radius:6px; border:1px solid #007bff; background:#f0f8ff; color:black;
}
.camera-buttons button:hover{background:#d0ebff;}
.camera-buttons button.selected{border:2px solid #007bff; background:#c0e0ff;}
.camera-container video,
.camera-container canvas{
  max-width:800px; height:auto; display:block;
}
</style>