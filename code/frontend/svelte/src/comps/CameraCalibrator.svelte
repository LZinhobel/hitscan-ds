<script lang="ts">
  ...
  import calibrationStore, { setCalibration } from "../stores/calibrationStore.svelte";
  ...
  function handleKey(e: KeyboardEvent) {
    ...
    if (mode === "rings") {
      switch (key) {
        ...
        case "e": case "+": case "=": r.radius += SCALE_STEP; break;
        case "q": case "-": r.radius = Math.max(10, r.radius - SCALE_STEP); break;
        ...
        case "r":
          if (currentRing + 1 < NUM_RINGS) {
            ...
            if (!isLoaded) {
             next.x = r.x; next.y = r.y;
             next.scaleX = r.scaleX; next.scaleY = r.scaleY;
            }
            currentRing++;
          } else {
...
<style>
  #compBody { height: 45vh; margin-bottom: 10vh; }
  #controls {
    display: flex; flex-direction: column; gap: 1rem;
    width: 18vw; height: 48vh; margin-left: 3vw;
    background-color: rgba(220, 161, 186, 0.6);
    padding: 1vh 1vw;
  }
  p { color: #d6d6d6; }
  .camera-buttons { display: flex; flex-direction: column; gap: 1rem; flex-wrap: wrap; }
  .camera-buttons button {
    cursor: pointer; border-radius: 6px; border: 2px solid #000;
    background: #862b52; color: #fff; height: 6vh;
    margin-right: 10%; margin-bottom: 3vh;
  }
  .camera-buttons button:hover { background: #af376b; }
  .camera-buttons button.selected {
    height: 10%; max-height: 10vh; max-width: 10vw;
    margin-bottom: 5%; color: black; background: #db4888;
  }
  #row { display: flex; flex-direction: row; }
  .camera-container video, .camera-container canvas {
    max-width: 800px; height: auto; display: block; object-fit: contain;
  }
</style>
